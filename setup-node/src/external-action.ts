...
async function cloneRepository(tempFolder: string, repositoryUrl: string) {
  try {
    ...
    core.error(err);
    throw new Error(
      "There was an error while trying to clone the action repository"
    );
  }
}
...
async function checkout(tempFolder: string, reference: string) {
  try {
    ...
    core.error(err);
    throw new Error(
      `There was an error while trying to checkout '${reference}'`
    );
  }
}
...
async function executeAction(actionFileFolder: string) {
  ...
  if (!("runs" in actionFileObject) || !("using" in actionFileObject.runs)) {
    throw new Error(
      `There was an error while paesing 'action.yml' file, missing 'runs.using'`
    );
  }
  ...
  if (!actionFileObject.runs.using.startsWith("node")) {
    throw new Error(
      `Unexpected value '${actionFileObject.runs.using}' for 'runs.using' in the 'action.yml' file`
    );
  }
